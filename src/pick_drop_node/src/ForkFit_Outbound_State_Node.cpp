...
class ForkFitOutboundStateNode : public LifecycleNode
{
private:
  // ===== Sensor State =====
  ...
  enum OUTBOUND_STATE {
    OUTBOUND_IDLE,
    OUTBOUND_PICKUP_PROCESSING,
    OUTBOUND_PICKUP_COMPLETE,
    OUTBOUND_DROP_COMPLETE
  };

  // Seq. 동작 (출고)
  OUTBOUND_STATE eState_{OUTBOUND_STATE::OUTBOUND_IDLE};
  OUTBOUND_STATE eStateOld_{static_cast<OUTBOUND_STATE>(-1)};
...
public:
  explicit ForkFitOutboundStateNode(
    const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
  : LifecycleNode("forkfit_outbound_state_node", options)
  {
    ...
    // Subscribers (LifecycleNode에서도 생성 가능)
  ...
  CallbackReturn on_configure(const rclcpp_lifecycle::State &)
  {
    ...
    eState_ = OUTBOUND_STATE::OUTBOUND_IDLE;
  ...
  CallbackReturn on_activate(const rclcpp_lifecycle::State &)
  {
    ...
    // activate 시점에 타이머 생성
    seqAction_ = create_wall_timer(100ms,
      std::bind(&ForkFitOutboundStateNode::OutboundAction, this));
  ...
  CallbackReturn on_cleanup(const rclcpp_lifecycle::State &)
  {
    ...
    seqAction_.reset();
    eState_ = OUTBOUND_STATE::OUTBOUND_IDLE;
    eStateOld_ = static_cast<OUTBOUND_STATE>(-1);
...
private:
  void OutboundAction()
  {
    ...
    if(eStateOld_ != eState_)
    {
        ...
        std::string logMsg = "[OUTBOUND] ";
        switch (eState_) {
            case OUTBOUND_STATE::OUTBOUND_IDLE:
            logMsg += "IDLE -> PROCESSING";
            break;
        
            case OUTBOUND_STATE::OUTBOUND_PICKUP_PROCESSING:
            logMsg += "PROCESSING -> COMPLETE";
            break;
            
            case OUTBOUND_STATE::OUTBOUND_PICKUP_COMPLETE:
            case OUTBOUND_STATE::OUTBOUND_DROP_COMPLETE:
    ...
    switch (eState_)
    {
      case OUTBOUND_STATE::OUTBOUND_IDLE:
      {
        ...
        if(newObjectState != lastObjectState_)
        {
            ...
            // 드롭 완료 (old=IN & new=OUT)
            if(OBJECT_OUT == newObjectState)
            {
                eState_ = OUTBOUND_STATE::OUTBOUND_DROP_COMPLETE;
                break;
            }
            // 픽업 (old=OUT & new=IN & 높이 기준 미달)
            else
            {
                eState_ = OUTBOUND_STATE::OUTBOUND_PICKUP_COMPLETE;
            }
        }
        // 픽업 Check (이전 OBJECT 상태가 Off 일 경우만)
        else if(OBJECT_OUT == lastObjectState_)
        {
            // 최신 값 갱신
            ...
            {
                eState_ = OUTBOUND_STATE::OUTBOUND_PICKUP_PROCESSING;
                break;
            }
        }
      }
      ...
      case OUTBOUND_STATE::OUTBOUND_PICKUP_COMPLETE:
      {
        ...
        // 물건을 집어서 내려와야하는데 물건이 없다면, pick-up 이 아닌 예외 발생
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<ForkFitOutboundStateNode>();
